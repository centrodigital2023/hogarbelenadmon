...
serve(async (req) => {
  ...
  try {
    ...
    const allowedRoles = ["super_admin", "coordinador", "enfermera"];
    const hasAccess = !!userRoles?.some((r: any) => allowedRoles.includes(r.role));
    if (!hasAccess) {
      return new Response(JSON.stringify({ error: "Acceso restringido a personal médico autorizado" }), {
...
Genera el informe completo, profesional y detallado.`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) return new Response(JSON.stringify({ error: "Créditos insuficientes para generar el reporte." }), { status: 402, headers: { ...corsHeaders, "Content-Type": "application/json" } });
