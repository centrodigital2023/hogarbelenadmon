...
serve(async (req) => {
  ...
  try {
    ...
    const allowedRoles = ["super_admin", "coordinador", "enfermera", "terapeuta", "psicologo"];
    const hasAccess = !!userRoles?.some((r: any) => allowedRoles.includes(r.role));
    if (!hasAccess) {
      return new Response(JSON.stringify({ error: "Acceso restringido a personal clínico autorizado" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
Genera el INFORME DE VALORACIÓN GERIÁTRICA INTEGRAL (HB-F22) completo siguiendo el formato indicado. Correlaciona la dependencia física con el estado cognitivo, nutricional y social. Redacta el Plan de Cuidado Personalizado basado en la normativa técnica del Ministerio de Salud de Colombia. Incluye Recomendaciones de Enfermería específicas para el nivel de fragilidad detectado.`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) return new Response(JSON.stringify({ error: "Créditos insuficientes para generar el reporte geriátrico." }), { status: 402, headers: { ...corsHeaders, "Content-Type": "application/json" } });
